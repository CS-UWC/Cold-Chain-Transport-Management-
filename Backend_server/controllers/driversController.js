...
const assign = require('../middleware/assigndriver');
...
//check is driver exists
    ...
    // console.log(email)
    console.log(drivers);
    drivers.logginStatus = true; // Replace with the new value
    await drivers.save();
    ...
    if (drivers && (password === drivers.password)) {
        const token = jwt.sign({
            ...
            orderId: drivers.orderId,
            role: drivers.role,
            logginStatus: drivers.logginStatus,
...
const getDriver = async (req, res) => {
    const driver = await Driver.findOne({where: {driverId: req.driver.driverId}});
    if (!driver) {
...
const getDriverOrders = async (req, res) => {
    ...
    orders.findAll({ where: {driverId: driverId } }).then((orders) => {
